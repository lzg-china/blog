...
```json
"scripts": {
  "start:db": "json-server -w db.json --port 3005",
...
② 通过泛型参数将要同步的数据的类型传递到 Sync 类中，使用泛型约束 save 方法的参数，使 Sync 类可重用。
...
> 目标：在用户表单类中实现点击按钮创建用户并将用户信息同步到服务端。
...
② 创建 UserContainer 视图类，在该类中准备 UserForm、UserShow 视图的挂载点
